...
$hasYandexSubscription=$_SESSION['yandex'];
...
// Запрос для получения отзывов о продукте с именами пользователей
$sql_reviews = "SELECT pr.comment, pr.rating, pr.created_at, u.username 
...
<head>
    ...
    <style>
        ...
        .main-content{
            ...
            border: 10px rgb(202, 202, 79) solid;
            margin: 1em auto;
...
<div class="page">
    ...
    <div id="product" >
        <img height="400px" src="<?php echo $product['image_url']; ?>" alt="<?php echo $product['name']; ?>">
        <div id="product-info">
            ...
            <h3>
            <?php 
                if ($product_id % 2 === 0 && $hasYandexSubscription) {
                    // Apply a 30% discount for products with an ID divisible by 2 and with a subscription
                    $discountedPrice = $product['price'] * 0.7;
                    echo "Цена со скидкой: " . $discountedPrice . "тг";
                } else {
                    // Display the regular price
                    echo "Цена: " . $product['price'] . "тг";
                }
            ?>                
        </h3>
            <div class="buttons">
            <?php echo "<button type='submit' onclick='openPopup(" . $product['id'] . ")' >Купить</button>" ?>
            <form method="post" action="addToCart.php" onsubmit="return checkLogin()">
        ...
        <div class="popup" id="buyPopup">
            ...
            <form id="creditCardForm" action="process_credit_card.php" method="post">
                <input type="hidden" id="ticketIdInput" name="product_id" value="">
                <label for="cardNumber">Номер карты:</label>
                <input type="text" id="cardNumber" name="creditcard" pattern="\d{16}" placeholder="Введите 16-значный номер" required>
                <br>

                <label for="expirationDate">Срок действия:</label>
                <input type="text" id="expirationDate" name="expirationDate" pattern="\d{2}/\d{2}" placeholder="ММ/ГГ" required>
                <br>

                <label for="cvv">CVV:</label>
                <input type="text" id="cvv" name="cvv" pattern="\d{3}" placeholder="Введите 3-значный код" required>
                <br>

                <label for="cardHolderName">Имя держателя карты:</label>
                <input type="text" id="cardHolderName" name="cardHolderName" placeholder="Введите имя" required>
                <br>
                ...
                <button class="buy-button" type="submit">Оплатить</button>
            </form>
            <button class="close-button" type="submit" onclick='closePopup()'>Закрыть</button>
            <?php
            if ($_SERVER["REQUEST_METHOD"] === "POST") {
                include("process_credit_card.php");
            }
            ?>
        </div>
        <div id="reviews">
            ...
            <div>
                <?php foreach ($reviews as $review) : ?>
                    <div class="review-container">
                        <p class="review-header">Пользователь: <?php echo $review['username']; ?></p>
        ...
        <div class="form-container">
            <form action="submit_review.php" method="post" onsubmit="return checkLogin()">
        ...
        function checkLogin() {
            <?php if (!isset($_SESSION['username'])) : ?>
                alert('Чтобы отправить отзыв, пожалуйста, войдите в систему или зарегистрируйтесь.');
                return false;
            <?php endif; ?>
            return true;
        }
