...
// Первоначальные значения фильтров
$category = isset($_GET['category']) ? $_GET['category'] : "дом";
$minPrice = 0;
$maxPrice = 1000000;
$novelty = "";
$productName = "";
...
if (isset($_GET['applyFilter'])) {
    // Использование переданных значений фильтров, если они были переданы
    $minPrice = isset($_GET['minPrice']) ? $_GET['minPrice'] : $minPrice;
    $maxPrice = isset($_GET['maxPrice']) ? $_GET['maxPrice'] : $maxPrice;
    $novelty = isset($_GET['novelty']) ? $_GET['novelty'] : $novelty;
    $productName = isset($_GET['productName']) ? $_GET['productName'] : $productName;
}

// Подготовка SQL запроса с учетом параметров фильтрации
$sql = "SELECT * FROM Products WHERE category = ? AND price >= ? AND price <= ?";

// Добавление условия для фильтрации по названию продукта
...
// Добавление параметра для названия продукта, если оно указано
if (!empty($productName)) {
    $productName = "%{$productName}%";
    $stmt->bind_param("sdds", $category, $minPrice, $maxPrice, $productName);
} else {
    ...
    ';
                ...
                <form action="filter.php" method="GET">
                    ...
                    <div class="filter-container">
                        ...
                        <input type="number" id="minPrice" name="minPrice" min="0" max="1000000" value="<?php echo $minPrice; ?>">
                        <label for="maxPrice">Цена до:</label>
                        <input type="number" id="maxPrice" name="maxPrice" min="0" max="1000000" value="<?php echo $maxPrice; ?>">

                        <label for="novelty">Новизна:</label>
                        <select id="novelty" name="novelty">
                            <option value="new" <?php echo ($novelty === 'new') ? 'selected' : ''; ?>>Новинки</option>
                            <option value="old" <?php echo ($novelty === 'old') ? 'selected' : ''; ?>>Старые</option>
                        </select>

                        <label for="productName">Название продукта:</label>
                        <input type="text" id="productName" name="productName" value="<?php echo $productName; ?>">

                        <button type="submit" name="applyFilter">Применить фильтр</button>
                    </div>
                </form>

    <!-- Карточки продуктов -->
    <?php
    echo "<h1 id='category_name' >$category</h1>";
?>
